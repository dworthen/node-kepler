...
kepler.md = function (fileObj) {
	if(fileObj.extname === '.md' || fileObj.extname === '.markdown') {
		fileObj['__content'] = marked(fileObj['__content']);
...
config['ignoredFiles'] = ['_*', '.*', '*.json', '*.log'];
config['fileFunctions'] = [kepler.md, kepler.template];
config['stylus'] = false;
config['ignoredRules'] = {
	'\\/' : '\\/',	
	'\\.' : '\\.',
...
kepler.setStylus = function (dir) {
	config['stylus'] = path.join(config['projectDirectory'], dir);
	return config['stylus'];
};
...
kepler.createFileObj = function (file, projectDirectory, destinationDirectory, layoutsDirectory) {
	...
	var fileObj = {__content: ''};
	if(statsMethods.isFile) {
		var fileContent = fs.readFileSync(file);
		if(pathObj.extname !== '.js' && pathObj.extname !== '.json') {
...
kepler.createFile = function (fileObj) {
	...
	if(fileObj.isDirectory) {
		if(!path.existsSync(fileObj.destination)) {
			kepler.mkdirpSync(fileObj.destination, 0755);
		}
	} else if (!path.existsSync(fileObj.destinationDirectory)) {
		kepler.mkdirpSync(fileObj.destinationDirectory, 0755);
	} else if (!path.existsSync(fileObj.destination)) {
