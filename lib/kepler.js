...
kepler.addIgnoredFiles = function (arr) {
	ignoredFiles = ignoredFiles.concat(arr);
};

kepler.setStylus = function (dir) {
	stylus = path.join(srcDir, dir);
};

kepler.getStylus = function () {
	return stylus;
};
...
kepler.createFile = function (srcFile, desFile) {
	var srcContent = fs.readFileSync(srcFile),
			extname    = path.extname(srcFile),
			html,
			propObj = {content: ''};
	if(extname !== '.js' && extname !== '.pdf' && extname !== '.png' && extname !== '.jpg') {
